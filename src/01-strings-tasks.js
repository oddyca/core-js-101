...
function extractNameFromTemplate(value) {
  return value.match(/\w+\s\w+\W$/)[0].slice(0, -1);
}
...
function encodeToRot13(str) {
  // let newStr = str.replaceAll(/\w/, )
  let newStr = `${str}`;
  newStr = newStr.replace(/[a-z]/gi, (letter) => String.fromCharCode(letter.charCodeAt() + (letter.toLowerCase() <= 'm' ? 13 : -13)));
  return newStr;
}
...
function isString(value) {
  const isIt = typeof value === 'string' || value instanceof String;
  return isIt;
}
...
function getCardId(value) {
  ...
  const indexedDeck = new Map();
  for (let i = 0; i < deck.length; i += 1) {
    indexedDeck[deck[i]] = i;
  }

  return indexedDeck[value];
}
